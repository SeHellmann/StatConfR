# Tests for ll_Mratio_cpp and ll_MratioF_cpp vs. R references
# ITG parameter vector: [log_sens x nCond | log_crit_A x nRatings-1 | theta | log_crit_B x nRatings-1 | log_m_ratio]

generate_itg_inputs <- function(nCond = 2, nRatings = 4, seed = 123) {
  set.seed(seed)
...
itg_edge_cases <- list(
  high_m_ratio = function(inputs) {
    inputs$p[inputs$nCond + 2 * inputs$nRatings] <- log(3)
    inputs
  },
  low_m_ratio = function(inputs) {
...
test_that("ll_Mratio_cpp matches R across configurations", {
  configs <- list(
...
test_that("ll_Mratio_cpp handles edge cases", {
...
test_that("ll_MratioF_cpp matches R across configurations", {
  configs <- list(
    list(nCond = 2, nRatings = 4, seed = 123),
    list(nCond = 1, nRatings = 4, seed = 456),
    list(nCond = 2, nRatings = 2, seed = 789),
...
test_that("ll_MratioF_cpp handles edge cases", {
  base <- generate_itg_inputs(nCond = 2, nRatings = 4, seed = 123)
  for (nm in names(itg_edge_cases)) {
    inputs <- itg_edge_cases[[nm]](base)
...
test_that("ll_Mratio_cpp matches R across parameter grid", {
  for (nCond in 1:3) {
    for (nRatings in 2:5) {
...
test_that("ll_MratioF_cpp matches R across parameter grid", {
  for (nCond in 1:3) {
    for (nRatings in 2:5) {
      for (seed in c(111, 222, 333)) {
        inputs <- generate_itg_inputs(nCond, nRatings, seed)
...
test_that("ll_Mratio_cpp is faster than R version", {
  skip_on_cran()
  skip_on_ci()
...
test_that("ll_MratioF_cpp is faster than R version", {
  skip_on_cran()
  skip_on_ci()
  inputs <- generate_itg_inputs(nCond = 3, nRatings = 5, seed = 123)
  n_iter <- 100
