# Tests for ll_SDTvarS_cpp vs. R reference (llSDTvarS)
# SDTvarS parameter vector: [sens x nCond (raw d', not log) | log_crit_A x nRatings-1 | theta |
#                            log_crit_B x nRatings-1 | log_variance_scale]
# The variance of the signal distribution scales with sensitivity:
#   sd = sqrt(1 + locB^2 * exp(variance_scale))

generate_sdtvars_inputs <- function(nCond = 2, nRatings = 4, seed = 123) {
  set.seed(seed)
  p <- numeric(nCond + 2 * nRatings)
  p[1:nCond] <- runif(nCond, 0.5, 2.5)  # raw sensitivity values, not log-transformed
  if (nRatings > 1) {
...
sdtvars_edge_cases <- list(
  ...
  high_sensitivity = function(inputs) {
    inputs$p[1:inputs$nCond] <- rep(3, inputs$nCond)
    inputs
  },
  low_sensitivity = function(inputs) {
    inputs$p[1:inputs$nCond] <- rep(0.3, inputs$nCond)
    inputs
  },
  high_variance = function(inputs) {
    inputs$p[inputs$nCond + 2 * inputs$nRatings] <- log(3.0)
    inputs
  },
  low_variance = function(inputs) {
    inputs$p[inputs$nCond + 2 * inputs$nRatings] <- log(0.2)
    inputs
  },
  variance_scale_zero = function(inputs) {
    inputs$p[inputs$nCond + 2 * inputs$nRatings] <- 0  # exp(0) = 1
    inputs
  },
...
test_that("ll_SDTvarS_cpp matches R across configurations", {
  configs <- list(
...
test_that("ll_SDTvarS_cpp handles edge cases", {
...
test_that("ll_SDTvarS_cpp responds to variance parameter changes", {
  ...
  expect_false(r1 == call_cpp(inputs2),
    info = "Likelihood should change when variance parameter changes")
})
...
test_that("ll_SDTvarS_cpp responds to sensitivity changes", {
  ...
  expect_false(call_cpp(inputs_lo) == call_cpp(inputs_hi),
    info = "Different sensitivities should produce different likelihoods")
})

test_that("ll_SDTvarS_cpp matches R across parameter grid", {
  for (nCond in 1:3) {
    for (nRatings in 2:5) {
...
test_that("ll_SDTvarS_cpp is faster than R version", {
  skip_on_cran()
  skip_on_ci()
