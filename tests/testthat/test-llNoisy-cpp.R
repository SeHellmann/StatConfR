# Tests for ll_Noisy_cpp vs. R reference (ll_Noisy)
# Noisy parameter vector: [log_sens x nCond | log_crit_A x nRatings-1 | theta | log_crit_B x nRatings-1 | log_sigma]
# R indexing bug skip nRatings=2 for comparison

generate_noisy_inputs <- function(nCond = 2, nRatings = 4, seed = 123) {
  set.seed(seed)
...
test_that("ll_Noisy_cpp matches R across configurations", {
  configs <- list(
  ...
  for (cfg in configs) {
    ...
    inputs <- generate_noisy_inputs(cfg$nCond, cfg$nRatings, cfg$seed)
    tol <- if (!is.null(cfg$tol)) cfg$tol else 1e-4
    result <- compare_likelihood_generic(ll_Noisy, ll_Noisy_cpp, inputs, tolerance = tol)
...
test_that("ll_Noisy_cpp handles edge cases", {
  base <- generate_noisy_inputs(nCond = 2, nRatings = 4, seed = 123)
  for (nm in names(noisy_edge_cases)) {
    inputs <- noisy_edge_cases[[nm]](base)
    result <- compare_likelihood_generic(ll_Noisy, ll_Noisy_cpp, inputs, tolerance = 1e-4)
...
test_that("ll_Noisy_cpp output is finite, non-negative, and deterministic", {
  inputs <- generate_noisy_inputs(nCond = 2, nRatings = 4, seed = 123)
  call_cpp <- function(inp) {
    ll_Noisy_cpp(inp$p, inp$N_SA_RA, inp$N_SA_RB, inp$N_SB_RA, inp$N_SB_RB, inp$nRatings, inp$nCond)
  }
  r1 <- call_cpp(inputs)
...
test_that("ll_Noisy_cpp matches R across parameter grid", {
  for (nCond in 1:3) {
    for (nRatings in 2:5) {
      if (nRatings == 2) next  # R indexing bug skip nRatings=2 for comparison
      for (seed in c(111, 222, 333)) {
        inputs <- generate_noisy_inputs(nCond, nRatings, seed)
        result <- compare_likelihood_generic(ll_Noisy, ll_Noisy_cpp, inputs, tolerance = 1e-3)
        expect_true(result$match,
...
test_that("ll_Noisy_cpp is faster than R version", {
  skip_on_cran()
  skip_on_ci()
  inputs <- generate_noisy_inputs(nCond = 3, nRatings = 5, seed = 123)
  n_iter <- 50
  r_time   <- system.time(for (i in seq_len(n_iter)) ll_Noisy(inputs$p, inputs$N_SA_RA, inputs$N_SA_RB, inputs$N_SB_RA, inputs$N_SB_RB, inputs$nRatings, inputs$nCond))["elapsed"]
  cpp_time <- system.time(for (i in seq_len(n_iter)) ll_Noisy_cpp(inputs$p, inputs$N_SA_RA, inputs$N_SA_RB, inputs$N_SB_RA, inputs$N_SB_RB, inputs$nRatings, inputs$nCond))["elapsed"]
  message(sprintf("ll_Noisy - R: %.3fs, C++: %.3fs, speedup: %.1fx", r_time, cpp_time, r_time / cpp_time))
