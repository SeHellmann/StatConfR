...
test_that("ll_Noisy_cpp matches R across configurations", {
  ...
  for (cfg in configs) {
    ...
    result <- compare_likelihood_generic("Noisy", ll_Noisy, inputs, tolerance = tol)
...
test_that("ll_Noisy_cpp handles edge cases", {
  ...
  for (nm in names(noisy_edge_cases)) {
    ...
    result <- compare_likelihood_generic("Noisy", ll_Noisy, inputs, tolerance = 1e-4)
  ...
  for (nCond in 1:3) {
    for (nRatings in 2:5) {
      for (seed in c(111, 222, 333)) {
        ...
        result <- compare_likelihood_generic("Noisy", ll_Noisy, inputs, tolerance = 1e-3)
        expect_true(result$match,
