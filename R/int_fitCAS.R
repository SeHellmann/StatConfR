...
fitCAS_fast <-
  function(N_SA_RA, N_SA_RB, N_SB_RA, N_SB_RB,
    ...
    inits <- prepare_accum_inits("CAS", nRatings, nCond, model_defs)
    ...
    # 5. Post Processing
    res <- process_accum_results(fit_res, "CAS", nRatings, nCond, nTrials, model_defs)
    return(res)
  }


fitCAS_regression <- function(data, formulas, nInits, nRestart) {
  
  # 1. Prepare Input
  prep <- preprocess_regression_input(data, formulas, model = "CAS", nInits = nInits)
  
  # 2. Optimization
  fn_ptr <- get_cas_regression_ptr()
