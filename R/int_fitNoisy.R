...
fitNoisy_fast <-
  function(N_SA_RA, N_SA_RB, N_SB_RA, N_SB_RB,
    ...
    inits <- prepare_accum_inits("GN", nRatings, nCond, model_defs)
    
    # 2. Compute Likelihoods on Grid (Using C++ for speed)
    fn_ptr <- get_noisy_ptr()
    ...
    # 5. Post Processing
    res <- process_accum_results(fit_res, "GN", nRatings, nCond, nTrials, model_defs)
    return(res)
  }


fitNoisy_regression <- function(data, formulas, nInits, nRestart) {
  
  # 1. Prepare Input
  prep <- preprocess_regression_input(data, formulas, model = "GN", nInits = nInits)
  
  # 2. Optimization
  fn_ptr <- get_noisy_regression_ptr()
