...
static double ll_Mratio_helper(const arma::vec& p, const ModelData& dat,
                               bool fleming) {
    ...
    std::vector<double> P_sara(nCond), P_sarb(nCond), P_sbra(nCond),
        P_sbrb(nCond);
    std::vector<double> d_RA_A2(nCond), d_RB_A2(nCond), d_RA_B2(nCond),
        d_RB_B2(nCond);
    ...
    for (int i = 0; i < nRatings; ++i) {
        for (int j = 0; j < nCond; ++j) {
            ...
            double p_sa_ra =
                std::max(P_sara[j] * (curr_RA_A2 - prev_RA_A2[j]) / d_RA_A2[j],
                         constants::MIN_P);
            double p_sa_rb =
                std::max(P_sarb[j] * (curr_RB_A2 - prev_RB_A2[j]) / d_RB_A2[j],
                         constants::MIN_P);
            double p_sb_ra =
                std::max(P_sbra[j] * (curr_RA_B2 - prev_RA_B2[j]) / d_RA_B2[j],
                         constants::MIN_P);
            double p_sb_rb =
                std::max(P_sbrb[j] * (curr_RB_B2 - prev_RB_B2[j]) / d_RB_B2[j],
                         constants::MIN_P);
...
static double ll_Mratio_regression_helper(const vec& p,
                                          const RegressionData& dat,
                                          bool fleming) {
    int n_meta = 1;
    ...
    auto calc_prob = [fleming](bool resp_A, double loc, double theta,
        ...
        double P1 = resp_A ? normcdf_cpp(theta - loc)
                           : (1.0 - normcdf_cpp(theta - loc));

        double D2 = resp_A ? normcdf_cpp(meta_c - loc2)
                           : (1.0 - normcdf_cpp(meta_c - loc2));
