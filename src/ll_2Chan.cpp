...
double ll_2Chan_cpp(const arma::vec& p, const ModelData& dat) {
    ...
    if (nRatings > 2) {
        ...
        arma::vec p_2 = arma::cumsum(arma::exp(
    ...
    for (int i = 0; i < nRatings; ++i) {
        for (int j = 0; j < nCond; ++j) {
            ...
            double p_sa_ra = std::max(P_sara[j] * (curr_RA_A2 - prev_RA_A2[j]),
                                      constants::MIN_P);
            double p_sa_rb = std::max(P_sarb[j] * (curr_RB_A2 - prev_RB_A2[j]),
                                      constants::MIN_P);
            double p_sb_ra = std::max(P_sbra[j] * (curr_RA_B2 - prev_RA_B2[j]),
                                      constants::MIN_P);
            double p_sb_rb = std::max(P_sbrb[j] * (curr_RB_B2 - prev_RB_B2[j]),
                                      constants::MIN_P);
...
double ll_2Chan_regression(const arma::vec& p, const RegressionData& dat) {
    ...
    auto calc_prob = [](bool resp_A, double loc, double theta,
        ...
        double p_chan1 = resp_A ? normcdf_cpp(theta - loc)
