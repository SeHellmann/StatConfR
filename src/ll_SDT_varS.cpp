...
double ll_SDTvarS_cpp(const arma::vec& p, const ModelData& dat) {
    ...
    const arma::vec sd = arma::sqrt(
        1 + arma::square(locB) * std::exp(p(nCond + nRatings * 2 - 1)));
    ...
    double negLogL = 0.0;
    std::vector<double> prev_RA_A(nCond), prev_RA_B(nCond), prev_RB_A(nCond),
        prev_RB_B(nCond);
    ...
    for (int i = 0; i < nRatings; i++) {
        for (int j = 0; j < nCond; j++) {
            ...
            double p_sa_ra =
                std::max(curr_RA_A - prev_RA_A[j], constants::MIN_P);
            double p_sa_rb =
                std::max(curr_RB_A - prev_RB_A[j], constants::MIN_P);
            double p_sb_ra =
                std::max(curr_RA_B - prev_RA_B[j], constants::MIN_P);
            double p_sb_rb =
                std::max(curr_RB_B - prev_RB_B[j], constants::MIN_P);
